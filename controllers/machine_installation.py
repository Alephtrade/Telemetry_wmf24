                ...
                if machine_response and machine_response["ip"] is not None:
                    ...
                    if not finder:
                        ...
                        #db_conn.connection.cursor().close()
                            db_conn.create_device(str(aleph_id), str(utc_calc(latitude, longitude)), str(machine_response["ip"]), str(machine_response["ProductName"]), str(1), str(data_for_request["full_serial"]), block_status)
                            db_conn.connection.cursor().close()
                    else:
                        print("finder")
                        #print(machine_response["ip"])
