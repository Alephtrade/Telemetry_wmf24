...
def test():
    ...
    for host in hosts["scan"]:
        if host != "10.8.0.1":
            ...
            response["aleph_id"] = "2345"
            db_conn.create_device(response["aleph_id"], utc_calc(machine_response["latitude"], machine_response["longitude"]), machine_response["ip"], machine_response["ProductName"], str(1))
