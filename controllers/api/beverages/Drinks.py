            ...
            if available_recipe is None:
                ...
                if received_recipes != available_recipe[4]:
                    if available_recipe[2] == drink["RecipeNumber"] and available_recipe[3] == drink["Name"] and available_recipe[5] == columns["coffee"]["count"] and available_recipe[6] == columns["coffee"]["weight"] and available_recipe[7] == columns["water"]["count"] and available_recipe[8] == columns["water"]["weight"] and available_recipe[9] == columns["milk"]["count"] and available_recipe[10] == columns["milk"]["weight"] and available_recipe[11] == columns["powder"]["count"] and available_recipe[12] == columns["powder"]["weight"] and available_recipe[13] == columns["foam"]["count"] and available_recipe[14] == columns["foam"]["weight"]:
                        ...
                        edited = device[1]
                        edited = {"device": device[1], available_recipe[2]: {"recipe_id": available_recipe[2], "edited_parts": {}}}
                        if str(available_recipe[3]) != str(drink["Name"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["recipe_alias"] = {"been": available_recipe[3], "now": drink["Name"]}
                        if int(available_recipe[5]) != int(columns["coffee"]["count"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["coffee_count"] = {"been": available_recipe[5], "now": columns["coffee"]["count"]}
                        if int(available_recipe[6]) != int(columns["coffee"]["weight"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["coffee_weight"] = {"been": available_recipe[6], "now": columns["coffee"]["weight"]}
                        if int(available_recipe[7]) != int(columns["water"]["count"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["water_count"] = {"been": available_recipe[7], "now": columns["water"]["count"]}
                        if int(available_recipe[8]) != int(columns["water"]["weight"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["water_weight"] = {"been": available_recipe[8], "now": columns["water"]["weight"]}
                        if int(available_recipe[9]) != int(columns["milk"]["count"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["milk_count"] = {"been": available_recipe[9], "now": columns["milk"]["count"]}
                        if int(available_recipe[10]) != int(columns["milk"]["weight"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["milk_weight"] = {"been": available_recipe[10], "now": columns["milk"]["weight"]}
                        if int(available_recipe[11]) != int(columns["powder"]["count"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["powder_count"] = {"been": available_recipe[11], "now": columns["powder"]["count"]}
                        if int(available_recipe[12]) != int(columns["powder"]["weight"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["powder_weight"] = {"been": available_recipe[12], "now": columns["powder"]["weight"]}
                        if int(available_recipe[13]) != int(columns["foam"]["count"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["foam_count"] = {"been": available_recipe[13], "now": columns["foam"]["count"]}
                        if int(available_recipe[14]) != int(columns["foam"]["weight"]):
                            edited["collector"][available_recipe[2]]["edited_parts"]["foam_weight"] = {"been": available_recipe[14], "now": columns["foam"]["weight"]}
