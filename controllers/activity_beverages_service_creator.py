...
def get_main_clean_stat(device):
    ...
    if len(unsent_disconnect_records) == 0:
        unsent_disconnect_records.append(
            (0, '', datetime.fromtimestamp(prev_hour.timestamp()), datetime.fromtimestamp(prev_hour.timestamp())))
    if len(unsent_records) == 0:
        unsent_records.append(
            (0, '', datetime.fromtimestamp(time_now.timestamp()), datetime.fromtimestamp(time_now.timestamp())))
    for disconnect_rec_id, disconnect_error_code, disconnect_start_time, disconnect_end_time in unsent_disconnect_records:
        ...
        if disconnect_end_time < date_end_prev_error:
            disconnect_end_time = date_end_prev_error
        if disconnect_start_time < date_end_prev_error:
            disconnect_start_time = date_end_prev_error
        for rec_id, error_code, start_time, end_time in unsent_records:
            ...
            if start_time < disconnect_start_time and end_time > disconnect_end_time:  # 3.4.3
                start_time = (disconnect_start_time - start_time)
                end_time = disconnect_end_time - end_time
            elif start_time < disconnect_start_time and end_time < disconnect_end_time:
                if end_time < disconnect_start_time:
                    start_time = disconnect_start_time
                    end_time = disconnect_start_time
                else:
                    end_time = disconnect_start_time
            if disconnect_start_time < start_time and end_time < disconnect_end_time:  # 3.4.3.1
                start_time = disconnect_end_time
                end_time = disconnect_end_time  # 3.4.3.2
            print(start_time)
            print(end_time)
